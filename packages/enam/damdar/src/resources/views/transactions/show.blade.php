    ...
    <div class="card card-default">
        <div class="card-heading clearfix">
            ...
            <div class="float-right">
                ...
                <form method="POST" action="{!! route('transactions.transaction.destroy', $transaction->id) !!}"
                      accept-charset="UTF-8">
                    <input name="_method" value="DELETE" type="hidden">
                    {{ csrf_field() }}
                    <div class="btn-group btn-group-sm" role="group">
                        <a target="_blank" href="{{ route('transactions.transaction.pdf',$transaction->id) }}"
                           class="btn btn-inverse-primary mr-2" title="Print Transaction">
                            <span class="mdi mdi-printer" aria-hidden="true"></span>
                            Print Transaction
                        </a>
                        <a href="{{ route('transactions.transaction.index') }}" class="btn btn-primary mr-2"
                           title="Show All Transaction">
                            <span class="mdi mdi-format-list-bulleted" aria-hidden="true"></span>
                            Show All Transaction
                        </a>

                        <a href="{{ route('transactions.transaction.create') }}" class="btn btn-success mr-2"
                           title="Create New Transaction">
                            <span class="mdi mdi-plus" aria-hidden="true"></span>
                            Create New Transaction
                        </a>

                        <a href="{{ route('transactions.transaction.edit', $transaction->id ) }}"
