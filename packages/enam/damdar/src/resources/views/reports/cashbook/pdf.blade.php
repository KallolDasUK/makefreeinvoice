...
@section('content')
    <center>
        <h2>Cash Book Report ({{ $branch_name??'All' }})</h2>
    ...
    width:100%;text-align: center">
        ...
        @foreach($txn_details as $txn_detail)
            ...
            <tr>
                ...
                <td style="max-width: 50px!important; word-wrap: break-word;text-align: start">{{ $txn_detail->ledgersName??'-' }}</td>
                ...
                @if($txn_detail->entry_type==\Enam\Acc\Utils\EntryType::$CR)
                    <td>{{ number_format($txn_detail->transaction->amount) }}</td>
                    <td>0</td>
                    @php($dr+=$txn_detail->transaction->amount)
                @else
                    <td>0</td>
                    <td>{{ number_format($txn_detail->transaction->amount) }}</td>
                    @php($cr+=$txn_detail->transaction->amount)
    ...
    <hr>
    <p style="float: right;font-weight: bold">Balance = {{ number_format($dr-$cr) }}</p>
