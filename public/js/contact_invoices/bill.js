    ...
    observe: {
        'bill_items': (newValue) => {
            ...
            for (let i = 0; i < bill_items.length; i++) {
                ...
                sub += (parseFloat(item.workers) || 0) * (parseFloat(item.daily_cost) || 0)* (parseFloat(item.working_days) || 0);
                ...
                if (item.product_id && tax_id) {
                    ...
                    let taxAmount = ((parseFloat(item.workers) || 0) * (parseFloat(item.daily_cost) || 0)* (parseFloat(item.working_days) || 0) * parseFloat(tax.value)) / 100;
