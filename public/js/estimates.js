...
var ractive = new Ractive({
    ...
    addEstimateItem() {
        ...
        ractive.push('estimate_items', copiedObject)
        let i = ractive.get('estimate_items').length - 1;
        ...
        $(currentSelectItem).select2({
            ...
            ractive.set(`estimate_items.${i}.product_id`, $(this).val())
            setTimeout(() => {
        ...
        $(`#itemTax${i}`).select2({
            ...
            let i = $(this).attr('index');
            ractive.set(`estimate_items.${i}.tax_id`, $(this).val())
    ...
    delete(index) {
        ractive.splice('estimate_items', index, 1);
    },
    observe: {
        'estimate_items': (newValue) => {
            let estimate_items = newValue;
            let sub = 0;
            ractiveExtra.set(`appliedTax`, [])
            // alert('sdlfk')
            for (let i = 0; i < estimate_items.length; i++) {
                let item = estimate_items[i];
...
function calculateOthers() {
    ...
    $('#estimate_items').val(JSON.stringify(ractive.get('estimate_items')))
...
$(document).ready(function () {
    ...
    // calculateOthers()
    ractive.set('estimate_items.0.test', '')
...
$(document).on('keyup', `.select2-search__field`, function (e) {
    if (e.which === 13) {
        ...
        ractive.set(`estimate_items.${i}.product_id`, e.target.value)
        $(`#${id}`).select2("close");
        // $('.rate').focus()
        $(`#row${i}`).find('.rate').focus()
