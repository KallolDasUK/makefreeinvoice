@extends('master.master-layout')

@section('content')

    <div class="card">

        <div class="card-header">

            <h5  class="my-1 float-left">{{ !empty($blogTag->name) ? $blogTag->name : 'Blog Tag' }}</h5>

            <div class="btn-group btn-group-sm float-right" role="group">

                <a href="{{ route('blog_tags.blog_tag.index') }}" class="btn btn-primary mr-2" title="Show All Blog Tag">
                    <i class=" fas fa-fw fa-th-list" aria-hidden="true"></i>
                    Show All Blog Tag
                </a>

                <a href="{{ route('blog_tags.blog_tag.create') }}" class="btn btn-success" title="Create New Blog Tag">
                    <i class=" fas fa-fw fa-plus" aria-hidden="true"></i>
                    Create New Blog Tag
                </a>

            </div>
        </div>

        <div class="card-body">

            @if ($errors->any())
                <ul class="alert alert-danger">
                    @foreach ($errors->all() as $error)
                        <li>{{ $error }}</li>
                    @endforeach
                </ul>
            @endif

            <form method="POST" action="{{ route('blog_tags.blog_tag.update', $blogTag->id) }}" id="edit_blog_tag_form" name="edit_blog_tag_form" accept-charset="UTF-8" class="form-horizontal">
            {{ csrf_field() }}
            <input name="_method" type="hidden" value="PUT">
            @include ('blog_tags.form', [
                                        'blogTag' => $blogTag,
                                      ])

                <div class="form-group">
                    <div class="col-md-offset-2 col-md-10">
                        <input class="btn btn-primary" type="submit" value="Update">
                    </div>
                </div>
            </form>

        </div>
    </div>

@endsection
