...
<div class="form-group row">
    ...
    <select class="form-control col-lg-4" id="payment_method_id" name="payment_method_id">
        ...
        @foreach ($paymentMethods as $key => $paymentMethod)
            <option
                value="{{ $key }}" {{ old('payment_method_id', optional($receivePayment)->payment_method_id) == $key ? 'selected' : '' }}>
                {{ $paymentMethod }}
            </option>
        @endforeach
    </select>
...
<div class="form-group row">
    ...
    <select class="form-control col-lg-4" id="deposit_to" name="deposit_to" required="true">
        <option value="" style="display: none;"
                {{ old('deposit_to', optional($receivePayment)->deposit_to ?: '') == '' ? 'selected' : '' }} disabled
                selected>Enter deposit to here...
        </option>
        @foreach (['0' => 'Petty Cash','1' => 'Bank Account 1','2' => 'Test 2'] as $key => $text)
            <option
                value="{{ $key }}" {{ old('deposit_to', optional($receivePayment)->deposit_to) == $key ? 'selected' : '' }}>
                {{ $text }}
            </option>
        @endforeach
    </select>
