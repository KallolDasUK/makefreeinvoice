...
<table class="table mt-5">
    ...
    <tr>
        ...
        <td class="border-0 pl-0" style="text-align: right">
            <h4 class="text-uppercase">
                <strong>{{ $invoice->name }}</strong>
            </h4>
            <p>{{ $invoice->name }} Date: <strong>{{ $invoice->getDate() }}</strong></p>
            @if($invoice->expires_on??false)
                <p>Expires On: <strong>{{ $invoice->expires_on }}</strong></p>
            @else
                <p>Due Date: <strong>{{ $invoice->invoice->invoice_date }}</strong></p>
            @endif
            <p>{{ $invoice->name }} Number <strong>{{ $invoice->getSerialNumber() }}</strong></p>
        </td>
    </tr>
    </tbody>
</table>
...
<table class="table">
    ...
    <tr>
        ...
        <th class="border-0 pl-0" style="text-align: right">
            <h3>To</h3>
        </th>
    </tr>
    </thead>
    ...
    <tr>
        ...
        <td class="border-0">
            @foreach($invoice->extras as $extra)
                <span> <b>{{ $extra->name }}</b> : {{ $extra->value }}</span> <br>
            @endforeach
